...
class PCBPlacementGenerator:
    ...
    def _generate_component_details_text(self, micro_regions: List[MicroRegion]) -> str:
        """
        生成组件详细信息的文本格式（类似KiCad解析格式）
        所有组件都在 top 层（F.Cu, F.Mask）
        """
        lines = []
        
        for micro in micro_regions:
            lines.append(f"[{micro.id} (Internal Context)]:")
            lines.append("- Entities:")
            
            for comp in micro.components:
                # 构建完整的组件ID：M{micro_num}_Comp_{original_id}
                micro_num = micro.id.split('_')[1] if '_' in micro.id else micro.id
                full_comp_id = f"M{micro_num}_{comp.id}"
                
                # 封装名称：config_library:footprint_type
                footprint_full = f"config_library:{comp.comp_type}"
                
                # 组件头信息
                lines.append(f"  {full_comp_id} ({footprint_full}):")
                lines.append(f"    Size: {comp.width:.6f}x{comp.height:.6f}, Rotation: {comp.rotation}")
                lines.append(f"    Pads:")
                lines.append(f"    {{")
                
                # 遍历所有焊盘
                for i, pin in enumerate(comp.pins, start=1):
                    # 焊盘层信息：全部使用贴片焊盘（F.Cu, F.Mask）
                    layers = "F.Cu, F.Mask"
                    
                    # 格式化焊盘信息
                    pad_line = (
                        f"      {i}: ["
                        f"rel: [{pin.rel_pos[0]:.2f}, {pin.rel_pos[1]:.2f}], "
                        f"size: [{pin.size[0]:.2f}, {pin.size[1]:.2f}], "
                        f"net: {pin.net}, "
                        f"rot: {pin.rotation}, "
                        f"layers: {layers}]"
                    )
            ...
            lines.append("")  # Micro 之间空一行
        
        return "\n".join(lines)


    def generate_single_circuit(self, circuit_id: str = None) -> Tuple[Dict, str]:
        """
        生成单个电路的 placement 和 netlist
        
        返回:
            Tuple[Dict, str]: (电路数据字典, 组件详情文本)
        ...
        # Step 5: 生成组件详情文本
        component_details_text = self._generate_component_details_text(micro_regions)
        
        return circuit_dict, component_details_text
    ...
    def _place_components_in_micro(self, micro: MicroRegion) -> List[Component]:
        ...
        for _ in range(num_components):
            ...
            # 计算包含焊盘的完整尺寸（需要先临时设置位置来计算）
            component.position = [0.0, 0.0]
            full_bbox = component.get_full_bbox()
            # 计算从元件中心到完整边界框各边的距离
            margin_left = -full_bbox[0]    # 中心到左边界的距离
            margin_right = full_bbox[2]    # 中心到右边界的距离
            margin_bottom = -full_bbox[1]  # 中心到下边界的距离
            margin_top = full_bbox[3]      # 中心到上边界的距离
            
            placed = False
            for _ in range(max_retries):
                # 使用完整边界框的 margin 来计算有效放置范围
                x_min = usable_bbox[0] + margin_left
                x_max = usable_bbox[2] - margin_right
                y_min = usable_bbox[1] + margin_bottom
                y_max = usable_bbox[3] - margin_top
    ...
    def _check_component_overlap(self, new_comp: Component, 
        ...
        """
        # 获取新元件的完整边界框（包含焊盘）
        new_bbox = new_comp.get_full_bbox()
        # 元件间最小间距
        margin = getattr(self.config, 'component_spacing', 0.3)
        
        for comp in existing:
            # 获取现有元件的完整边界框（包含焊盘）
            comp_bbox = comp.get_full_bbox()
            
            # 检查两个边界框是否重叠（考虑间距）
            if not (new_bbox[2] + margin < comp_bbox[0] or   # new 在 comp 左边
                comp_bbox[2] + margin < new_bbox[0] or    # comp 在 new 左边
                new_bbox[3] + margin < comp_bbox[1] or    # new 在 comp 下边
                comp_bbox[3] + margin < new_bbox[1]):     # comp 在 new 下边
                return True  # 有重叠
