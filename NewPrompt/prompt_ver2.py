...
B2. DYNAMIC CHANNEL_WIDTH CALCULATION (SILENT STEP):
    Before assigning dx/dy, analyze the nets between Source and Target:
    - If connected by POWER/GND (High Current): Calculated_Channel_Width = 8.0.
    - If connected by BUS/MULTIPLE SIGNALS (>2 nets): Calculated_Channel_Width = 6.0.
    - If connected by SINGLE SIGNAL: Calculated_Channel_Width = 3.0.
    - If NO direct connection: Calculated_Channel_Width = 3.0 (Minimum Isolation).  
...
################################################################################
{
  "UCG_Graph": {
    ...
    "level_1_details": {
      ...
      "Micro_2": { "nodes": [], "spatial_topology": [] },
      "Micro_3": { "nodes": [], "spatial_topology": [] },
      "Micro_N": { "nodes": [], "spatial_topology": [] }
    }
  }
}
