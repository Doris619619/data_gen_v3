...
1. NODES:
   - COMPONENT: {id, type, w, h, pads, rot}
2. SPATIAL TOPOLOGY (Relative Positioning)
   - Edge Syntax: {"type": "SPATIAL", "source": "A", "target": "B", "dx": float, "dy": float}
3. ROUTING GRAPH (Connectivity)
   - Edge Syntax: {"source": "NodeA.Pin", "via_space_node": {"id": "SN_X", "size": "Small/Medium/Large"}, "target": "NodeB.Pin"}
...
A3. HIERARCHY ISOLATION (STRICT BOUNDARIES):
...
A4. NO SPURIOUS CONNECTIONS: Do not force a connection just to fill the JSON. If a net only appears on one pin, no path_sequence unless it connects to Global.
A5. ROTATION SYSTEM: Angles are in Degrees (0.0, 90.0, 180.0, 270.0/-90.0). 
...
B2. STRICT NON-OVERLAPPING RULE (HARD CONSTRAINT):
...
B5. COORDINATE CONSISTENCY (CHAIN RULE):
...
B6. DIRECTED ACYCLIC TOPOLOGY (DAG) & FULL ANCHORING:RULE: 
...
--- [C.  ROUTING RESOURCES RULES ] ---
C1. SPACE NODE OCCUPANCY:
...
C2. FIXED CHANNEL_WIDTH MAPPING: channel width should map with the types of space node
...
C4. ONE-TO-MANY CONNECTIVITY (Shared Nodes): A single Pad CAN be used in multiple path_sequence entries if it belongs to a shared net (e.g., for GND, ensure all GND pads connect to common GND resources).
C5. EXHAUSTIVE NET MAPPING: Scan EVERY pad of EVERY component. If a pad has a net name that exists on any other pad or micro-interface within the same level, a routing_resources entry MUST be generated.
