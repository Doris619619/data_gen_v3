...
class UCGExporter:
    ...
    def export(self, placement: Dict, routed_connections: List[RoutedConnection],
        ...
        # 如果metadata中没有micro_info，从pad_infos中推断
        if not micro_info:
            exit("Error: micro_info is missing in metadata.")
            # micro_info, micro_partition = self._infer_micro_info_from_pads(
            #     pad_infos, components, placement
            # )
    ...
    def _infer_micro_info_from_pads(self, pad_infos: Dict[str, PadInfo],
                                     components: Dict,
                                     placement: Dict) -> Tuple[Dict, Dict]:
        """从pad_infos中推断Micro信息"""
        # 收集所有Micro ID
        micro_components = defaultdict(list)
        micro_bounds = defaultdict(lambda: [float('inf'), float('inf'), 
                                            float('-inf'), float('-inf')])
        
        for pad_key, pad_info in pad_infos.items():
            micro_id = pad_info.micro_id or "Micro_1"
            comp_id = pad_info.component_id
            
            if comp_id not in [c for m_comps in micro_components.values() for c in m_comps]:
                micro_components[micro_id].append(comp_id)
            
            # 更新bbox
            bounds = micro_bounds[micro_id]
            bounds[0] = min(bounds[0], pad_info.center_x - pad_info.width/2)
            bounds[1] = min(bounds[1], pad_info.center_y - pad_info.height/2)
            bounds[2] = max(bounds[2], pad_info.center_x + pad_info.width/2)
            bounds[3] = max(bounds[3], pad_info.center_y + pad_info.height/2)
        
        # 如果没有找到任何Micro信息，创建默认的
        if not micro_components:
            canvas = placement.get("canvas", {})
            micro_info = {
                "Micro_1": {
                    "bbox": [0, 0, canvas.get("width", 100), canvas.get("height", 100)],
                    "num_components": len(components),
                    "area": canvas.get("width", 100) * canvas.get("height", 100)
                }
            }
            micro_partition = {"Micro_1": list(components.keys())}
            return micro_info, micro_partition
        
        # 构建micro_info
        micro_info = {}
        micro_partition = {}
        
        for micro_id, comp_ids in micro_components.items():
            bounds = micro_bounds[micro_id]
            # 添加一些边距
            margin = 2.0
            bbox = [
            ...
            micro_partition[micro_id] = comp_ids
    ...
    def _build_level_0(self, micro_info: Dict, components: Dict,
        ...
        """构建level_0_global"""
        self._reset_space_node_counter()  # level_0独立计数
    ...
    def _build_micro_spatial_topology(self, micro_info: Dict,
        ...
        for i, micro_id_1 in enumerate(micro_ids):
            for micro_id_2 in micro_ids[i+1:]:
                ...
                pos1 = micro_positions.get(micro_id_1, (0, 0))
                pos2 = micro_positions.get(micro_id_2, (0, 0))
    ...
    def _build_level_1(self, micro_info: Dict, components: Dict,
        ...
        for micro_id in micro_info.keys():
            ...
            level_1[micro_id] = self._build_micro_detail(
                micro_id, micro_components, connections, pad_infos, comp_to_micro
            )
    ...
    def _build_component_spatial_topology(self, micro_components: Dict,
        ...
        for i, comp_id_1 in enumerate(comp_ids):
            for comp_id_2 in comp_ids[i+1:]:
                pos1 = comp_positions.get(comp_id_1, (0, 0))
                pos2 = comp_positions.get(comp_id_2, (0, 0))
